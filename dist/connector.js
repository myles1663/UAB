...
export class UABConnector {
    ...
    async connect(target) {
        ...
        if (typeof target === 'number') {
            ...
            if (profile) {
                ...
                if (!detected)
                    throw new Error(`No detectable app at PID ${target}`);
        ...
        if (!app.pid)
            throw new Error(`App "${app.name}" has no known PID. Run scan() first.`);
    ...
    async enumerate(pid, maxDepth = 3) {
    ...
    ensureStarted() {
        if (!this.started)
            throw new Error('Connector not started. Call start() first.');
    }
    ensureConnected(pid) {
        this.ensureStarted();
        const route = this.router.getRoute(pid);
        if (!route)
            throw new Error(`Not connected to PID ${pid}. Call connect() first.`);
    }
    extractExe(app) {
