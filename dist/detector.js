...
function getRunningProcesses() {
    try {
        ...
        const rawList = runPSJson(script, 15000);
        const processes = [];
 ...
 */
function batchGetProcessModules(pids) {
 ...
 */
function batchGetWindowTitles(pids) {
    const result = new Map();
    if (pids.length === 0)
        return result;
    try {
...
export class FrameworkDetector {
    ...
    async detectAll() {
        ...
        const candidates = processes.filter(p => p.pid >= 100 && !SYSTEM_PROCESSES.has(p.name.toLowerCase()));
    ...
    async findByName(name) {
        ...
        const matches = all.filter(app => app.name.toLowerCase().includes(nameLower) ||
