...
export function registerUABCommands(bot) {
    ...
    bot.command('apps', async (ctx) => {
        ...
        try {
            ...
            if (apps.length === 0) {
                await ctx.api.editMessageText(ctx.chat.id, status.message_id, '🖥️ <b>No controllable apps detected</b>\n\n' +
                    '💡 <i>Tip: Electron apps need</i> <code>--remote-debugging-port=9222</code> <i>flag</i>', { parse_mode: 'HTML' });
                return;
            }
            ...
            await ctx.api.editMessageText(ctx.chat.id, status.message_id, `❌ Scan failed: ${err instanceof Error ? err.message : err}`);
        }
    });
    ...
    bot.command('appconnect', async (ctx) => {
        ...
        try {
            ...
            await ctx.api.editMessageText(ctx.chat.id, status.message_id, `✅ <b>Connected!</b>\n\n` +
                `📱 PID: <code>${result.pid}</code>\n` +
                `🔧 Method: ${result.method}\n\n` +
                `💡 <i>Use /ui to browse elements, /click to interact</i>`, { parse_mode: 'HTML' });
        }
        catch (err) {
            await ctx.api.editMessageText(ctx.chat.id, status.message_id, `❌ Connection failed: ${err instanceof Error ? err.message : err}`);
        }
    });
    // ─── /appdisconnect — Disconnect from an app ─────────────────
    ...
    bot.command('ui', async (ctx) => {
        ...
        try {
            ...
            if (elements.length === 0) {
                ...
                await ctx.api.editMessageText(ctx.chat.id, status.message_id, `🔍 No elements matching: ${filterDesc}`);
                return;
            }
            ...
            await ctx.api.editMessageText(ctx.chat.id, status.message_id, `❌ Query failed: ${err instanceof Error ? err.message : err}`);
        }
    });
    // ─── /click — Click a UI element ──────────────────────────────
    ...
    bot.command('appstate', async (ctx) => {
        ...
        try {
            ...
            if (state.modals.length > 0)
                text += `🔲 Modals: ${state.modals.length} open\n`;
    ...
    bot.command('keypress', async (ctx) => {
        ...
        if (!key) {
            await ctx.reply('⌨️ Usage: <code>/keypress &lt;key&gt;</code>\n\n' +
                'Examples:\n• /keypress Enter\n• /keypress Tab\n• /keypress F5\n• /keypress a', { parse_mode: 'HTML' });
            return;
        }
        const connections = uab.getConnections();
        if (connections.length === 0) {
    ...
    bot.command('hotkey', async (ctx) => {
        const args = ctx.match?.trim();
        if (!args) {
            await ctx.reply('⌨️ Usage: <code>/hotkey &lt;key1+key2+...&gt;</code>\n\n' +
                'Examples:\n• /hotkey ctrl+s\n• /hotkey ctrl+shift+p\n• /hotkey alt+f4\n• /hotkey ctrl+c', { parse_mode: 'HTML' });
            return;
        }
    ...
    bot.command('appwin', async (ctx) => {
        ...
        if (!action || !['min', 'max', 'restore', 'close', 'move', 'resize'].includes(action)) {
            await ctx.reply('🪟 Usage: <code>/appwin &lt;action&gt; [params]</code>\n\n' +
                'Actions:\n' +
                '• /appwin min — Minimize\n' +
                '• /appwin max — Maximize\n' +
                ...
                '• /appwin resize 800 600 — Resize to w×h', { parse_mode: 'HTML' });
            return;
        }
        const connections = uab.getConnections();
        if (connections.length === 0) {
    ...
    bot.command('screenshot', async (ctx) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                const filePath = result.result || result.path || outPath;
            ...
            await ctx.api.editMessageText(ctx.chat.id, status.message_id, `❌ Screenshot error: ${err instanceof Error ? err.message : err}`);
        }
    });
    ...
    bot.command('chain', async (ctx) => {
        ...
        if (!json) {
            await ctx.reply('🔗 <b>Action Chains</b>\n\n' +
                ...
                '</pre>\n\n' +
                'Step types: action, wait, conditional, delay, keypress, hotkey, typeText', { parse_mode: 'HTML' });
            return;
        }
        let chain;
        try {
        ...
        try {
            ...
            for (const step of result.steps.slice(0, 10)) {
                ...
                if (step.error)
                    text += `   ❌ ${escapeHtml(step.error.substring(0, 100))}\n`;
            }
            if (result.error) {
            ...
            await ctx.api.editMessageText(ctx.chat.id, status.message_id, `❌ Chain error: ${err instanceof Error ? err.message : err}`);
        }
    });
}
