 ...
 */
import { CDPConnection } from '../electron/cdp.js';
 ...
 */
function getUserDataDir(processName, pid) {
    try {
...
async function closeBrowserGracefully(pid, processName) {
    ...
    for (let i = 0; i < 16; i++) {
        ...
        catch { /* tasklist failed, assume closed */
            return;
        }
    }
 ...
 * Returns the new PID.
 */
async function relaunchWithDebugPort(exePath, processName, userDataDir, profileDir, port) {
        ...
        if (!app.connectionInfo?.debugPort) {
            ...
            if (!cdpReady) {
                ...
                if (discovered) {
                    ...
                    if (!ready) {
                        throw new Error(`Browser relaunched (PID ${newPid}) but CDP not available on port ${actualPort} after 15s.\n` +
 ...
 */
export function getBrowserDisplayName(processName) {
...
class BrowserConnection {
    ...
    get connected() { return this.cdp.connected; }
    // ─── Core UAB API ──────────────────────────────────────────
    async enumerate() {
    ...
    async populateBoundsRecursive(elements, depth, maxDepth) {
        if (depth > maxDepth)
            return;
    ...
    async doSetCookie(params) {
        if (!params?.cookieName)
            return { success: false, error: 'cookieName is required' };
        try {
            // Get current URL if domain not specified
            let url = params.url;
            if (!url && !params.domain) {
                url = await this.cdp.evaluate('window.location.href');
            }
            const cookieParams = {
            ...
            if (params.path)
                cookieParams.path = params.path;
            if (params.secure !== undefined)
                cookieParams.secure = params.secure;
            if (params.httpOnly !== undefined)
                cookieParams.httpOnly = params.httpOnly;
            if (params.sameSite)
                cookieParams.sameSite = params.sameSite;
            if (params.expires)
                cookieParams.expires = params.expires;
    ...
    async doClearCookies(params) {
        try {
            if (params?.domain) {
                // Clear cookies for a specific domain
                const result = await this.cdp.send('Network.getCookies');
                const cookies = result.cookies || [];
                const domainCookies = cookies.filter(c => c.domain?.includes(params.domain));
    ...
    // ═══════════════════════════════════════════════════════════
    async doGetStorage(storageType, params) {
        try {
            if (params?.storageKey) {
                // Get specific key
                const value = await this.cdp.evaluate(`${storageType}.getItem(${JSON.stringify(params.storageKey)})`);
                return {
                    success: true,
    ...
    async doSetStorage(storageType, params) {
        if (!params?.storageKey)
            return { success: false, error: 'storageKey is required' };
        try {
            await this.cdp.evaluate(`${storageType}.setItem(${JSON.stringify(params.storageKey)}, ${JSON.stringify(params.storageValue || '')})`);
            return {
                success: true,
    ...
    async doDeleteStorage(storageType, params) {
        if (!params?.storageKey)
            return { success: false, error: 'storageKey is required' };
        try {
            await this.cdp.evaluate(`${storageType}.removeItem(${JSON.stringify(params.storageKey)})`);
            return {
                success: true,
    ...
    async doClearStorage(storageType) {
        try {
    ...
    async doFocus(nodeId) {
        await this.cdp.send('DOM.focus', { nodeId });
        return { success: true };
    }
    async doHover(nodeId) {
    ...
    async doExpandCollapse(nodeId, action) {
        const result = await this.cdp.send('DOM.resolveNode', { nodeId });
        const objectId = result.object?.objectId;
        if (!objectId)
            return { success: false, error: 'Cannot resolve node' };
    ...
    async doInvoke(nodeId, params) {
        ...
        return { success: true, result: invokeResult.result?.value };
    }
    // ═══════════════════════════════════════════════════════════
    // SCREENSHOT / KEYBOARD / WINDOW (same as Electron plugin)
    // ═══════════════════════════════════════════════════════════
    async doScreenshot(params) {
        try {
            const result = await this.cdp.send('Page.captureScreenshot', { format: 'png', quality: 100 });
            const data = result.data;
            if (!data)
                return { success: false, error: 'CDP returned no screenshot data' };
        ...
        try {
            ...
            for (const k of keys) {
                ...
                if (lower === 'ctrl' || lower === 'control') {
                    modifiers |= 2;
                    modKeyNames.push('Control');
                }
                else if (lower === 'alt') {
                    modifiers |= 1;
                    modKeyNames.push('Alt');
                }
                else if (lower === 'shift') {
                    modifiers |= 8;
                    modKeyNames.push('Shift');
                }
                else if (lower === 'meta' || lower === 'win') {
                    modifiers |= 4;
                    modKeyNames.push('Meta');
                }
                else
                    nonModKeys.push(k);
            }
    ...
    mapKeyToCDP(key) {
        ...
        if (CDP_KEY_MAP[lower])
            return CDP_KEY_MAP[lower];
        if (key.length === 1) {
...
`;
    ...
    async queryCDP(cssSelector, selector) {
        ...
        for (const nid of nodeIds.slice(0, selector.limit || 100)) {
            ...
            if (el && this.matchesSelector(el, selector))
                elements.push(el);
        }
        return elements;
    }
    ...
    ensureConnected() {
        if (!this.connected)
            throw new Error(`Not connected to ${this.app.name}`);
    }
}
