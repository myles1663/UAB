...
export class VisionPlugin {
    ...
    async connect(app) {
        ...
        if (!analyzer.available) {
            throw new Error('Vision fallback requires ANTHROPIC_API_KEY. Set it in .env or environment.');
        }
...
class VisionConnection {
     ...
     */
    executeAction(element, action, params) {
     ...
     */
    handleWindowAction(action, params) {
        switch (action) {
            case 'minimize':
     ...
     */
    filterElements(elements, selector) {
        return elements.filter(el => {
            if (selector.type && el.type !== selector.type)
                return false;
            if (selector.visible !== undefined && el.visible !== selector.visible)
                return false;
            if (selector.enabled !== undefined && el.enabled !== selector.enabled)
                return false;
            if (selector.label) {
                if (!el.label.toLowerCase().includes(selector.label.toLowerCase()))
                    return false;
            }
            if (selector.labelExact) {
                if (el.label !== selector.labelExact)
                    return false;
            }
            if (selector.labelRegex) {
                try {
                    if (!new RegExp(selector.labelRegex, 'i').test(el.label))
                        return false;
                }
                catch {
                    return false;
