 ...
 */
export function doubleClickAt(pid, x, y) {
    const script = `${foregroundScript(pid)}
 ...
 */
export function rightClickAt(pid, x, y) {
    const script = `${foregroundScript(pid)}
 ...
 */
export function hoverAt(pid, x, y) {
    const script = `${foregroundScript(pid)}
 ...
 */
export function dragPath(pid, path, stepDelay = 10, button = 'left') {
...
`;
    try {
        return runPSJsonInteractive(script, 30000);
    }
    catch (err) {
        return { success: false, error: err instanceof Error ? err.message : String(err) };
    }
 ...
 */
export function scrollAt(pid, x, y, amount) {
 ...
 */
export function sendKeypress(pid, key) {
 ...
 */
export function sendHotkey(pid, keys) {
 ...
 */
export function typeTextAt(pid, x, y, text) {
...
`;
    try {
        return runPSJsonInteractive(script, 15000);
    }
    catch (err) {
        return { success: false, error: err instanceof Error ? err.message : String(err) };
    }
 ...
 */
export function windowAction(pid, action) {
    const cmdMap = {
 ...
 */
export function captureScreenshot(pid, outputPath) {
...
`;
    try {
        const raw = runPSJsonInteractive(script, 20000);
        if (raw.success) {
            return {
                success: true,
                path: raw.path,
