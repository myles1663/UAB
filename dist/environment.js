...
function getSessionId() {
    try {
        const output = execSync('powershell -NoProfile -Command "(Get-Process -Id $PID).SessionId"', { encoding: 'utf-8', timeout: 5000 });
...
function canReachDesktop(sessionId) {
    if (sessionId > 0)
        return true; // Already in desktop session
    // Session 0: try to verify Task Scheduler can bridge to Session 1
    try {
        execSync('schtasks /Query /TN "\\Microsoft\\Windows\\Defrag\\ScheduledDefrag" >nul 2>&1', { timeout: 3000 });
 ...
 */
export function getDefaults(mode) {
