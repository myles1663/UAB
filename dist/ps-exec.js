 ...
 */
function execPSFile(script, timeoutMs) {
    ensureTempDir();
    const tempFile = join(TEMP_DIR, `uab-${randomUUID().substring(0, 8)}.ps1`);
    try {
        writeFileSync(tempFile, script, 'utf-8');
        return execSync(`powershell -NoProfile -ExecutionPolicy Bypass -File "${tempFile}"`, { encoding: 'utf-8', timeout: timeoutMs, maxBuffer: 10 * 1024 * 1024 });
    }
    finally {
        try {
            unlinkSync(tempFile);
 ...
 */
function execPSInteractive(script, timeoutMs) {
    ensureTempDir();
...
`;
    try {
        ...
        execSync(`schtasks /Create /TN "${taskName}" /SC ONCE /ST 00:00 /IT /F /TR "${psPath} ${args}" 2>nul`, { encoding: 'utf-8', timeout: 5000 });
        execSync(`schtasks /Run /TN "${taskName}" 2>nul`, { encoding: 'utf-8', timeout: 5000 });
        // Poll for completion
        ...
        // Cleanup
        try {
            execSync(`schtasks /Delete /TN "${taskName}" /F 2>nul`, { timeout: 3000 });
        }
        catch { /* ignore */ }
        try {
 ...
 */
export function runPSJson(script, timeoutMs = 15000) {
 ...
 */
export function runPSRaw(script, timeoutMs = 15000) {
 ...
 */
export function runPSJsonInteractive(script, timeoutMs = 15000) {
 ...
 * Falls back to direct execution when already in an interactive session.
 */
export function runPSRawInteractive(script, timeoutMs = 15000) {
    if (isSession0()) {
        return execPSInteractive(script, timeoutMs).trim();
    }
    return execPSFile(script, timeoutMs).trim();
}
//# sourceMappingURL=ps-exec.js.map