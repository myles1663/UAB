...
`.trim();
// ─── CLI-Specific Prompt ───────────────────────────────────────
function cliToolsGuide(cliPath) {
    return `
 ...
 */
export function getAgentPrompt(mode = 'mcp', options = {}) {
    const { port = 3100, cliPath = 'node dist/uab/cli.js', includeReference = true, includeExamples = true, } = options;
    const parts = [CORE_PHILOSOPHY];
    if (includeReference) {
        switch (mode) {
            case 'mcp':
 ...
 */
export function getClaudeMdSnippet(cliPath = 'node dist/uab/cli.js') {
    return `
 ...
 */
export function getMcpConfig(serverPath = 'dist/uab/mcp-server.js') {
    return {
        mcpServers: {
            'desktop-control': {
                command: 'node',
