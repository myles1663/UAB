 ...
 */
export async function findCoworkSkillsDir() {
    const home = homedir();
        ...
        for (const session of readdirSync(sessionsDir, { withFileTypes: true })) {
            ...
            for (const workspace of readdirSync(sessionPath, { withFileTypes: true })) {
                ...
                if (existsSync(coworkPlugins)) {
                    const skillDir = join(coworkPlugins, 'marketplaces', COWORK_MARKETPLACE, COWORK_PLUGIN_NAME, 'skills', SKILL_NAME);
                    paths.push(skillDir);
                }
            }
        }
 ...
 */
export function writeSkillToAllLocations(cliSkillPath, coworkPaths, content) {
    let cli = false;
    let cowork = 0;
